...
function displayQuestion() {
    ...
    let html = `<h3>Pertanyaan ${currentState.currentQuestion + 1}</h3>
...
function submitQuiz() {
    ...
    document.getElementById('playerInfo').innerHTML = `
        <p>Nama: ${currentState.playerName}</p>
        <p>NIM: ${currentState.playerNIM}</p>
    `;

    document.getElementById('scoreBreakdown').innerHTML = '<h3>Detail Skor:</h3>' + 
        breakdown.map(item => `
            <p>Pertanyaan ${item.question}: ${item.points}/${item.maxPoints} poin</p>
        `).join('');

    document.getElementById('totalScore').innerHTML = `<h3>Total Skor: ${totalScore} poin</h3>`;
